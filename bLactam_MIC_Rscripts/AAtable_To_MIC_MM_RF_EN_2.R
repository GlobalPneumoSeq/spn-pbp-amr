...
PBP_AA_TO_MIC2<- function(cwd){
  ...
  source(file.path(data_dir, "blosum_utils.R"))
  library("methods")
  ...
  colAA=5:918
  m1=read.csv(file.path(dbdir, "train_PT_AA_table.csv"), colClasses="character")
  n1=dim(m1)[1] 
  ...
  m4=read.csv(file.path(dbdir, "Ref_PBPtype_MIC.csv"), colClasses="character")
  ...
  ## RandomForest
  ## Re-level predictors
  m1=read.csv(file.path(dbdir, "NR_Varsites_Randomforest.csv"), colClasses="character")
  colAA=colnames(m1)
  ...
  blosum_path=file.path(data_dir, "BLOSUM62.csv")
  m3=read.csv(blosum_path, colClasses="character", header=T)
  ...
  for (j1 in 1:n2)
  {
    ...
    if (length(x3)>0) 
    {
      for (j2 in x3)
      {
        x4=choose_training_residue(j2, x1, m3)
        x5=m2.1[, j1]
        m2.1[j2==x5, j1]=x4
        print(c(j1, x4))
      }       
